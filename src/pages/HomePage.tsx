...
const HomePage: React.FC = () => {
  ...
  return (
    <>
        ...
        <div className="flex flex-col md:flex-row lg:flex-row w-[100%] items-center justify-between gap-[20px] lg:gap-[60px] my-[40px] px-[0px] lg:px-[20px]">

          <div className="w-[100%] md:w-[95%] h-[180px] border rounded-lg p-[15px] lg:p-[10px] w-[90%]">
            <h1 className='font-bold text-[14px] lg:text-[16px]'>{leftText}</h1>
          </div>

          <div className="grid grid-cols-2 sm:flex sm:flex-col lg:flex lg:flex-col items-center gap-[15px] lg:gap-[25px] my-[0px] w-[80%] mx-auto">
              <button onClick={handleButtonClick1} className="px-[10px] md:px-[20px] py-[10px] border border-white cursor-pointer rounded-lg text-[10px] md:text-lg hover:bg-blue-500">Submit Transaction</button>
              <button onClick={handleButtonClick2} className="px-[10px] md:px-[20px] py-[10px] border border-white cursor-pointer rounded-lg text-[10px] md:text-lg hover:bg-blue-500">Confirm Transaction</button>
              <button onClick={handleButtonClick3} className="px-[10px] md:px-[20px] py-[10px] border border-white cursor-pointer rounded-lg text-[10px] md:text-lg hover:bg-blue-500">Execute Transaction</button>
              <button onClick={handleButtonClick4} className="px-[10px] md:px-[20px] py-[10px] border border-white cursor-pointer rounded-lg text-[10px] md:text-lg hover:bg-blue-500">Revoke Confirmation</button>
          </div>

          <div className="md:w-[95%] h-[180px] border rounded-lg p-[15px] lg:p-[10px] w-[90%]">
              <h1 className='font-bold text-[14px] lg:text-md'>{rightText}</h1>
          </div>
        </div>

        <button onClick={navigateToSubmit} className="w-[90%] lg:w-[40%] px-[20px] py-[10px] border border-white cursor-pointer rounded-lg bg-blue-500 ">Get Started</button>
		</div>
    </>
  );
};
